...
const state = {
  ...
  currentPlan: null,
  // Andrew, 2026-08-04: bumped on every setMetricConfig call; the plan
  // records which version it was generated against (see setPlan) so any
  // view can tell whether currentPlan reflects the metric weights/enabled
  // state currently in Metric Center, or a stale snapshot from before the
  // last change -- weight changes take effect in scoring immediately, but
  // currentPlan is only ever regenerated by an explicit action (Generate
  // Plan, a store switch, Reset All to AI), never automatically.
  metricsConfigVersion: 0,
  planMetricsConfigVersion: null,
  ready: false,
...
export function setPlan(plan) {
  ...
  state.planMetricsConfigVersion = state.metricsConfigVersion;
  bus.emit('plan:updated', plan);
}

// True once a metric weight/enabled state has changed since currentPlan was
// generated -- the plan still reflects the OLD scoring config until
// something explicitly regenerates it (Generate Plan, a store switch, Reset
// All to AI). Callers combine this with their own storeId check (see
// planogramViewer.js/optimizationEngine.js/setOverview.js) since currentPlan
// itself is scoped to whichever store it was last generated for.
export function isPlanStale() {
  return !!state.currentPlan && state.planMetricsConfigVersion !== state.metricsConfigVersion;
}
