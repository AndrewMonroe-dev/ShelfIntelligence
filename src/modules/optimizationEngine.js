...
import { generatePlan, THIN_SECTION_WIDTH_FT } from '../optimize/placementSolver.js?v=20260805';
import { getPhysicalWidthFt } from '../optimize/shelfPosition.js?v=20260805';

// Andrew, 2026-08-04: a metric weight/enabled change takes effect in
// scoring immediately, but the plan shown here is only ever regenerated by
// an explicit action (Generate Plan, a store switch, Reset All to AI) --
// nothing pushes a live update to an already-rendered plan. Surfaces that
// gap instead of leaving it silent, per store.isPlanStale().
  ...
  return `
    <div class="card stale-plan-banner" style="margin-bottom:14px;border:1px solid var(--warning);background:rgba(245,158,11,.14);">
      <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap;">
        <span>&#9888; A metric weight or enabled state has changed in <a href="#metric-center">Metric Center</a> since this plan was generated -- scores and selection below may be stale.</span>
        <button class="btn btn-primary stale-plan-regenerate-btn">Regenerate Now</button>
      </div>
    </div>
  `;
}
...
export function mount(el) {
  ...
  function bindPlanOutputListeners(output, physicalWidthFt) {
    output.querySelector('.stale-plan-regenerate-btn')?.addEventListener('click', () => {
