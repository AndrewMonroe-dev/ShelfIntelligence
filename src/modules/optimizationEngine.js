...
import { THIN_SECTION_WIDTH_FT } from '../optimize/placementSolver.js?v=20260807b';
import { getPhysicalWidthFt } from '../optimize/shelfPosition.js?v=20260807b';
...
export function mount(el) {
  ...
  function renderSectionCard(section, physicalWidthFt) {
    ...
    return `
      <div class="card" data-section-key="${section.key}">
        ...
        ${section.type === 'merged'
          ? `<div style="margin-top:10px;font-size:11.5px;color:var(--text2);">Combined section (each category here is â‰¤${THIN_SECTION_WIDTH_FT}ft) -- resize the individual categories in Set Layout.</div>`
  ...
  function render() {
    ...
    // Andrew, 2026-08-07: auto-regenerate whenever there's no plan for this
    // store yet, OR any plan-generation input has changed since currentPlan
    // was last generated (store.isPlanStale()) -- Set Layout allocations,
    // target SKU count, section multipliers, case-only mode, metric weights,
    // or fixture edits. Safe to do silently here: currentPlan is a pure
    // derived structure, nothing on this page is hand-edited outside of
    // what regeneration itself recomputes, so there's no user work to lose.
    let plan = currentPlan && currentPlan.storeId === selectedStoreId ? currentPlan : null;
    if (!plan || store.isPlanStale()) plan = store.regeneratePlan(selectedStoreId);
    ...
    el.querySelector('.case-only-toggle').addEventListener('change', (e) => {
      ...
      const freshPlan = store.regeneratePlan(selectedStoreId);
      output.innerHTML = renderPlan(freshPlan, physicalWidthFt);
      bindPlanOutputListeners(output, physicalWidthFt);
    });
