...
import { computeBaseline } from './digitalTwin.js?v=20260807c';
import { calibrateVelocity, predictPlanTotals, compareToBaseline } from './predictor.js?v=20260807c';
...
export function runScenario(scenario, store, allSkus, baseMetricsConfig, targetSkuCount, bottleDimensions, sales, sectionMultipliers, sectionAllocations, sizePackage, caseOnlyMode, overrides) {
  ...
  const plan = generatePlan(store, allSkus, scenarioMetricsConfig, targetSkuCount, bottleDimensions, sectionAllocations, sectionMultipliers, sizePackage, caseOnlyMode, overrides);
...
export function runAllScenarios(scenarios, store, allSkus, baseMetricsConfig, targetSkuCount, bottleDimensions, sales, sectionMultipliers, sectionAllocations, sizePackage, caseOnlyMode, overrides) {
  return scenarios.map((scenario) =>
    runScenario(scenario, store, allSkus, baseMetricsConfig, targetSkuCount, bottleDimensions, sales, sectionMultipliers, sectionAllocations, sizePackage, caseOnlyMode, overrides)
  );
}
